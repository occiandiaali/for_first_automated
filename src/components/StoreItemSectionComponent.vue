...
async function postToArchive(id:string|undefined) {
 ...
 try {
  await axios.post(`${process.env.VUE_APP_API_BASE_URL}/api/user/archive`, { itemId: id }, {withCredentials:true})
  .then(response => {
...
async function deleteFromOrders(str: string|undefined) {
 ...
 try {
  const response = await axios.delete(`${process.env.VUE_APP_API_BASE_URL}/api/user/storefront/${str}`, {withCredentials:true});
  alert(`${response.data}`)
 } catch (error) {
